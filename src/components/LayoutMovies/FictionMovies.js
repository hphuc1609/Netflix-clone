...
function FictionMovies() {
    const { fictionMovies } = useSelector((state) => state.infoMovies);
    const windowWidth = useViewport();
    const dispatch = useDispatch();

    return (
        <div className={cx('container')}>
            <h1 className={cx('title')}>Phim Viễn Tưởng</h1>
            {fictionMovies && fictionMovies.length > 0 ? (
                <div
